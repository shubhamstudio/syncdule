...
const ListView = ({ setCreatePostModalOpen }: {
    ...
    queries: [
      ...
      },
      {
        ...
        queryFn: async () => {
          ...
          if (channelIds.length > 0) params.append("channelIds", channelIds.join(","))
          const res = await fetch(`/api/post/totals?${params.toString()}`)
          if (!res.ok) throw new Error(await getApiErrorMessage(res, "Unable to load schedule totals"))
          return res.json()
        },
        staleTime: 30_000,
        refetchOnWindowFocus: false,
  ...
  const publishPostMutation = useMutation({
    mutationFn: async (postId: string) => {
      ...
      if (!res.ok) throw new Error(await getApiErrorMessage(res, "Unable to publish post"));
      return res.json();
    },
    onSuccess: () => {
    ...
    <>
      <div className="flecx flex-col h-full pt-3">
        ...
        <div className="flex-1 p-6">
            <div className="max-w-[960px] mx-auto w-full space-y-2">
            {isPending ? (
              ...
              <EmptyState
                icon={LayoutList}
                title={`No ${activeTab === "queue" ? "scheduled" : activeTab} posts yet`}
                description="Connect a channel and create your first post to start building a consistent publishing rhythm."
                action={<Button size="lg" onClick={() => setCreatePostModalOpen(true)}><Plus className="size-4" /> Create Post</Button>}
              />
            ) : (
              <div className="w-full space-y-5">
                {groupPosts.map((group) => (
                  <div key={group.key}>
                    <h2 className="font-mono text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground">{group.label}</h2>
                    <div className="space-y-6">
                      {group.posts.map((post) => {
                        ...
                        lg:grid-cols-[120px_minmax(0,1fr)]">
                            <div>
                              <h5 className="font-mono text-xs font-medium tracking-wide text-foreground">
                            ...
                            <Card variant="interactive" className="gap-0 py-0">
                            ...
                            md:grid-cols-[minmax(0,1fr)_250px]">
                                ...
                                <div className="max-h-[165px] overflow-hidden rounded-md
                  border bg-muted/40">
                                  {previewImage ? (
                                    <img
                                      src={previewImage}
