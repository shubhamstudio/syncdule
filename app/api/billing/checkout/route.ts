...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isPlanId(body.plan)) {
      return NextResponse.json({ error: "Choose a valid monthly plan" }, { status: 400 });
    }

    const providerPlanId = planProviderId(body.plan);
    if (!providerPlanId) {
      return NextResponse.json({ error: "This plan is not ready for checkout yet. Please try again shortly." }, { status: 503 });
    }

    const { insforge: admin, userId } = await getAuthenticatedInsforgeAdminClient();
    if (!admin || userId !== billing.userId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    // Razorpay evaluates subscription RLS with the caller's user token; app-owned
    // billing rows continue to use the server-only admin client.
    const { insforge: payments, userId: paymentUserId } = await getInsforgeServerClient();
    if (paymentUserId !== billing.userId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    ...
    if (orderError || !internalOrder) {
      return NextResponse.json({ error: "Billing storage is not ready. Please try again shortly." }, { status: 503 });
    }

    const user = await currentUser();
    const { data, error } = await payments.payments.razorpay.createSubscription(RAZORPAY_ENVIRONMENT, {
    ...
    if (!providerSubscriptionId) {
      return NextResponse.json({ error: "Unable to prepare recurring checkout" }, { status: 502 });
    }

    await admin.database
      .from("billing_orders")
      .update({ provider_subscription_id: providerSubscriptionId, updated_at: new Date().toISOString() })
      .eq("id", internalOrder.id)
      .eq("user_id", billing.userId);
